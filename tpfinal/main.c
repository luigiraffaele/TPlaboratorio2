...
int consulaTitulo(char file[],char titulo[]){ ///Busca una pelicula por titulo,la muestra y retorna su ID
    int id = -1;
    int flag = 0;
    int control=1;
    pelicula aux;
...
void consultaGenero(char file[], char genero[]){ ///Busca todas las peliculas de un genero dentro del archivo y las muestra
    ...
    if(archi!=NULL){
        while(control > 0){
            control = fread(&aux,sizeof(pelicula),1,archi);
            if(strcmpi(aux.genero,genero)==0 && control >0){
...
///===== Funciones Ver Pelicula ==================///

pelicula buscarPeliculaYRetornar(char fileP[], int idP){ ///BUsca una pelicula en el archivo por su ID y la retorna
    int flag = 0;
    pelicula aux;
    FILE*archi = NULL;
...
void viendoTituloPelicula(char fileP[], int idP){
    pelicula aux;
    FILE*archi = NULL;
    archi = fopen(fileP,"rb");
    if(archi != NULL){
            if(existePeliculaID(fileP,idP)==1){
                aux = buscarPeliculaYRetornar(fileP,idP);
                printf("\nViendo \"%s\" \n",aux.nombrePelicula);
            }
    }
    fclose(archi);
...
int cargarPeliculasVistas(int vistas[], int dim, int idP){///Carga el arreglo de peliculas vistas del usuario
    int i = 0;
    int flag = 0;
...
void verPelicula(char fileU[], char fileP[], int idU, int idP){
    usuario user;
    pelicula peli;
    int flag = 0;
    FILE*archi = NULL;
    if(existePeliculaID(fileP,idP)==1){
        ...
        if(archi!=NULL){
            while(flag == 0){
                if(fread(&user,sizeof(usuario),1,archi)>0 && user.idUsuario == idU){
                    user.cantVistas = cargarPeliculasVistas(user.peliculasVistas,30,idP);
                    flag = 1;
                }
            }
        ...
        viendoTituloPelicula(fileP,idP);
        fclose(archi);

    }else{
        printf("Pelicula no existente\n");
    }
}

int existePeliculaID(char fileP[], int id){
    pelicula aux;
    int existe = 0;
    FILE*archi = NULL;
    archi = fopen(fileP,"rb");
    if(archi != NULL){
        while(fread(&aux,sizeof(pelicula),1,archi)>0 && existe == 0){
            if(aux.idPelicula == id){
                existe = 1;
            }
        }
...
void recomendarPeliculas(char fileP[],usuario auxUser){
int drama=0;
int accion=0;
int crime=0;
int i;
...
while(control>0){
            control = fread(&auxPeli,sizeof(pelicula),1,archi);
            if(auxUser.peliculasVistas[i]==auxPeli.idPelicula){
                if(strcmpi(auxPeli.genero,"drama")==0){
                    drama++;

                }
                if(strcmpi(auxPeli.genero,"accion")==0){
                    accion++;

                }
                if(strcmpi(auxPeli.genero,"crime")==0){
                    crime++;
        ...
        offset = ftell(archi);
        fseek(archi, sizeof(pelicula), SEEK_SET);
        i++;
    ...
    if(archi!=NULL){
        while(control > 0){
            control = fread(&aux,sizeof(pelicula),1,archi);
            if(strcmpi(aux.genero,"accion")==0){
                printf("* %s    ID:%i\n",aux.nombrePelicula,aux.idPelicula);
                accion--;
            }
        }   }
...
void mostrarUsuario(usuario user){ ///recibe un usuario y muestra su contenido
    ...
    printf("Nacimiento: %i\n",user.anioNacimiento);
    printf("Pais: %s\n",user.pais);
    printf("ID Peliculas vistas: ");
    mostarPeliculasVistas(user.peliculasVistas,30);
    printf("\nCantidad vistas: %i \n",user.cantVistas);
    printf("Eliminado: %i\n\n",user.eliminado);
}

void mostarPeliculasVistas(int vistas[], int cant){
    int i = 0;
    while(i<cant && vistas[i]!= 0){
        printf("%i, ",vistas[i]);
        i++;
    }
}
        ...
        while(flag == 0){
            if(fread(&aux,sizeof(usuario),1,archi)>0 && aux.idUsuario == id){
                ...
                while(continuar == 's'){
                    ...
                    break;
                    case 2:
                        printf("Pass (10 caracteres): "); ///PENDIENTE
                        fflush(stdin);
                        gets(&temp);
                        pasarContraseniaAMatrix(temp,auxMatrix);
...
void printNombrePelicula(char fileP[],int idP){
    int flag = 0;
    pelicula aux;
    FILE*archi = NULL;
    archi = fopen(fileP,"rb");
...
void historial(char fileP[], usuario aux){
    int i = 0;
    while(i<aux.cantVistas){
        printNombrePelicula(fileP,aux.peliculasVistas[i]);
        i++;
    }
}
...
void menuUsuario(char fileP[],char fileU[],int id){///Muestra el menu de usuario y sus opciones
    ...
    if(aux.genero == 'm'){ ///Sistema inteligente de reconocimiento de genero, casi hacker, ponele(?)
        printf("= Bienvenido, %s =\n\n",aux.nombreUsuario);
    } else {
        printf("= Bienvenida, %s =\n\n",aux.nombreUsuario);
    }
    ...
    case 1: ///Mostrar perfil del usuario
        while(continuar == 'n'){
            printf("= Perfil =\n\n");
            mostrarUsuario(aux);
            printf("Modificar informacion s/n: ");
            fflush(stdin);
            scanf("%c",&continuar);
            if(continuar == 's'){
                system("cls");
                modificarUsuario(fileU,aux.idUsuario);
                mostrarUsuarioEnArchivo(fileU,aux.idUsuario);
            }
            ...
            case 1:
                ...
                if(continuar=='s'){
                    while(continuar=='s'){
                        ...
                        printf("Ingrese ID: ");
                        scanf("%i",&idP);
                        verPelicula(fileU,fileP,id,idP);
            ...
            case 3:
                ...
                while(continuar == 'n'){
                    ...
                    while(continuar == 's'){
                        ...
                        fflush(stdin);
                        gets(&titulo);
                        idP = consulaTitulo(fileP,titulo);
                        ///buscarPeliculaYRetornar(fileP,idP);
                        printf("Ver pelicula s/n ");
                        fflush(stdin);
                        scanf("%c",&continuar);
                        if(continuar=='s'){
...
int validacionAdmin(){
    ...
    gets(&contrasenia);
        ...
        while( valido<0){
            ...
            if(strcmpi("admin",username)==0){


                if (strcmpi("1q2w3e4r5t",contrasenia)==0){
                    valido=1;
                }
            }
        }
